...
def generate_date_of_birth():
    center_date = datetime(1998, 1, 1)  # Środek rozkładu w wieku 22 lat względem dzisiejszej daty
    std_deviation_years = 15  # Odchylenie standardowe dla wieku (dowolna wartość do określenia zakresu wieku)

    while True:
        # Generowanie losowej liczby z rozkładu normalnego
        random_years = int(np.random.normal(loc=0, scale=std_deviation_years))

        # Obliczanie daty urodzenia
        birth_date = center_date - timedelta(days=random_years * 365)  # Przybliżenie 1 roku = 365 dni

        # Sprawdzenie czy wiek jest w odpowiednim zakresie
        if 15 <= (datetime.now() - birth_date).days // 365 <= 60:
            break

    return birth_date.strftime("%Y-%m-%d")

# Przykład użycia
random_date_of_birth = generate_date_of_birth()
print(random_date_of_birth)
